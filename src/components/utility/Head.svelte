...
<svelte:head>
  <title>{$page.data.title} | rubes.dev</title>
  ...
  <meta property="twitter:url" content="https://rubes.dev{$page.url.pathname.toString()}" />
  <meta property="twitter:image" content="https://rubes.dev/opengraph-image.jpg" />
  <meta name="twitter:title" content={$page.data.title} />
  <meta name="twitter:description" content={$page.data.description} />
  {@html `  <script type="application/ld+json">{
     "@context": "https://schema.org",
     "@type": "Website",
     "name": "${$page.data.title} | rubes.dev",
     "url": "https//rubes.dev${$page.url.pathname}",
     "logo": "https://rubes.dev/android-chrome-512x512.png"
     }</script>`}
</svelte:head>
